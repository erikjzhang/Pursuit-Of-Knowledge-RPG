...
classdef ModifiedSimpleGameEngine < handle
    properties
    ...
    methods
        function obj = ModifiedSimpleGameEngine(sprites_fname, sprite_height, sprite_width, zoom, background_color)
            % simpleGameEngine
            % Input: 
        ...
        function [key,isKeyDown] = getKeyboardInput(obj)
            %The function asks if an input is being pressed, and if it is,
            %returns the key.
            %isKeyDown returns TRUE if a key is being pressed and FALSE is a key is not being pressed
            %key returns A CHARACTER VARIABLE, NOT A STRING, the key if a
            %key is being pressed and 0 if one is not being pressed
            
            % Bring this scene to focus
            figure(obj.my_figure);
            
            %sets the current character to 0
            set(obj.my_figure,'CurrentChar','0');
            isKeyDown = false;
            pause(.15)
            %asks if a key is being pressed
            key = get(obj.my_figure,'CurrentChar');
           
            %sets isKeyDown based off of the value of key
            if (key == '0')
                isKeyDown = false;
            else
                isKeyDown = true;
